...
func main () {
	...
	gpid := unix.Getpgrp()
	fmt.Println("proc a group pid: ", gpid)

	err := unix.Setpgid(0,0)
	if err != nil {
		fmt.Println("set group process id error", err)
		os.Exit(1)
	}
	fmt.Println("proc_a group pid after being set: ", unix.Getpgrp())

	err = unix.Setpgid(0,1234)
	if err != nil {
		fmt.Println("set group process id error", err)
		os.Exit(1)
	}
	fmt.Println("proc_a group pid after being set 2: ", unix.Getpgrp())
